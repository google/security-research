"""Data-quality checks for `all-calls.ql` SARIF output (`runs`, `results`, `locations`, `edges` tables)."""
...
def test_all_calls_edges_and_locations_populated(all_calls_stats, kernel):
    """Reference 6.1.111 DB contains 1,829,002 call edges and 3,555,034 locations."""
    ...
    assert edges >= 300000, f"only {edges:,} callgraph edges in all-calls (expected >= 300,000)"
    assert locations >= 600000, f"only {locations:,} locations in all-calls (expected >= 600,000)"
...
def test_all_calls_messages_and_uris_valid(all_calls_stats):
    ...
    report(
        ...
        {"sample URIs checked": len(uris), "call target messages ('call to ...')": len(call_msgs)},
    )
    assert len(uris) > 0, "no location URIs found in all-calls"
    assert len(call_msgs) > 0, "no 'call to <func>' target location messages found in all-calls"
...
def test_all_calls_distribution(all_calls_stats, baseline_path):
    ...
    report("all_calls distribution", {"edges": edges, "baseline edges": base["edges"], "ratio": f"{ratio:.2f}x"})
    assert ratio >= 0.75, f"all_calls edge count dropped below 75% of baseline ({edges} vs {base['edges']})"
