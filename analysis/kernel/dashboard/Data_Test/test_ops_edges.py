...
from common import canonical_path, load_baseline, pct, report, top_counts


def test_ops_targets_not_empty(ops_targets, kernel):
    report(f"ops_targets rows [{kernel}]", {"rows": len(ops_targets)})
    assert ops_targets, "no ops-table edges -- indirect call resolution produced nothing"
...
def test_ops_targets_fields_are_populated(ops_targets):
    ...
    report("field population",
           {"blank parent": blank_parent, "blank field": blank_field,
...
def test_ops_targets_cover_known_ops_structs(ops_targets):
    """The well-known dispatch tables should all appear; absence means the
    aggregate-literal matching missed a whole class of indirect calls."""
    parents = {r["parent"] for r in ops_targets}
    expected = {"file_operations", "inode_operations"}
    present = expected & parents
    report("ops struct coverage",
    ...
    assert present, f"none of {sorted(expected)} appear among {len(parents)} parent structs"


def test_ops_target_line_ranges_are_ordered(ops_targets):
    bad = [r for r in ops_targets
           if r["target_start"].isdigit() and r["target_end"].isdigit()
           and int(r["target_start"]) > int(r["target_end"])]
    report("target line ranges", {"start > end": len(bad), "pct": f"{pct(len(bad), len(ops_targets)):.2f}%"})
    # Up to ~0.15% of kernel ops callbacks (e.g. SHOW_CPU_ATTR macros in drivers/base/cpu.c) span header/macro boundaries
    assert pct(len(bad), len(ops_targets)) < 0.5


def test_ops_targets_resolve_to_known_functions(ops_targets, function_locations):
    """A resolved target must be a function the extractor recorded; otherwise the
    edge points at a declaration whose body was never extracted."""
    ...
    report("targets vs function_locations",
           {"distinct targets": len(targets), "present %": f"{rate:.1f}",
...
def test_ops_targets_distribution(ops_targets, baseline_path):
    ...
    report("distribution", {"rows": len(ops_targets),
                            "baseline rows": base.get("rows")})
    if base.get("rows"):
        ratio = len(ops_targets) / base["rows"]
        assert ratio >= 0.75, f"ops_targets row count dropped below 75% of baseline ({len(ops_targets)} vs {base['rows']})"
