"""Data-quality checks for `syscall-node-locs.ql` and joined `syscall_node` locations."""
...
def test_syscall_node_locs_not_empty(syscall_node_locs, kernel):
    report(f"syscall_node_locs rows [{kernel}]", {"rows": len(syscall_node_locs)})
    assert len(syscall_node_locs) >= 50000, f"only {len(syscall_node_locs)} function locations extracted"


def test_syscall_node_locs_ranges_and_files(syscall_node_locs):
    bad_ranges = [
        r for r in syscall_node_locs
        if is_int(r.get("sl", "")) and is_int(r.get("el", ""))
...
def test_function_location_files_agree(syscall_node, function_locations):
    """function_location's file prefix in joined `syscall_node` is what prolog.sql joins on."""
    by_name = {}
    for row in function_locations:
        by_name.setdefault(row["function_name"], set()).add(canonical_path(row["file_path"]))
    ...
    report(
        ...
        {"checked unique functions": checked, "agree %": f"{pct(agree, checked):.1f}"},
    )
    assert checked > 0
