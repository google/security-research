"""Data-quality checks for the `configs` table (`kernel-configs-needed.ql` output).

Validates 5-column preprocessor branch tracking (`config, path, ifdef, endif, else_`).
"""
from __future__ import annotations

from common import is_int, pct, report, top_counts


def test_configs_not_empty(configs, kernel):
    report(f"configs rows [{kernel}]", {"total": len(configs)})
    assert len(configs) > 1000, f"only {len(configs)} config preprocessor blocks extracted"
...
def test_configs_bounds_are_ordered(configs):
    ...
    bad_endif = [
        r for r in configs
        if is_int(r["ifdef"]) and is_int(r["endif"])
        and int(r["ifdef"]) >= int(r["endif"])
    ]
    bad_else = [
        r for r in configs
        if is_int(r["ifdef"]) and is_int(r["endif"]) and is_int(r["else_"])
        and int(r["else_"]) != 0
    ...
    assert not bad_else, f"{len(bad_else)} config blocks have else_ outside (ifdef, endif)"
...
def test_configs_core_symbols_present(configs):
    ...
    report(
        "core config coverage",
        {
            ...
            "top configs": top_counts(configs, "config", n=5),
        },
    )
    assert not missing, f"expected core kernel configs missing: {sorted(missing)}"
