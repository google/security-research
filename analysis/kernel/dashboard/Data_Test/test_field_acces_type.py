"""Data-quality checks for the `field_access` table (`field-acces-type.ql` output)."""
...
def test_field_access_not_empty(field_access, kernel):
    ...
    assert len(field_access) >= 200000, f"only {len(field_access)} field access rows extracted"


def test_field_access_types_complete(field_access):
    """Must extract all three access categories: 'read', 'write', and 'exec' (function-pointer calls)."""
    counts = Counter(r["type"] for r in field_access)
    report(
        "field access type breakdown",
        {"read": counts.get("read", 0), "write": counts.get("write", 0), "exec": counts.get("exec", 0)},
    )
    ...
    assert counts.get("exec", 0) > 1000, "insufficient 'exec' (indirect call) field accesses"
...
def test_field_access_fields_and_locations_valid(field_access):
    ...
    blank_loc = sum(1 for r in field_access if not r["location"] or ":" not in r["location"])
    report(
        "field_access completeness",
        {"blank field": blank_field, "blank parent": blank_parent, "invalid location": blank_loc},
    )
    assert blank_loc == 0
...
def test_field_access_distribution(field_access, baseline_path):
    ...
    report("field_access distribution", {"rows": len(field_access), "baseline rows": base["rows"], "ratio": f"{ratio:.2f}x"})
    assert ratio >= 0.75, f"field_access row count dropped below 75% of baseline ({len(field_access)} vs {base['rows']})"
