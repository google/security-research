...
class TestCheckPrivilege(
    ...
    def test_format_capability(self):
        """Verify human-readable formatting of capability checks."""
        cap_map = check_privilege.load_capability_map(self.conn)
        self.assertEqual(
        ...
        self.assertEqual(
            check_privilege.format_capability(
                "ns_capable", "12", cap_map=cap_map
            ),
            "ns_capable(CAP_NET_ADMIN)",
        )
        self.assertEqual(
            check_privilege.format_capability(
                "ns_capable", "cap", cap_map=cap_map
            ),
            "ns_capable(cap)",
        )

    def test_deduplicate_gates(self):
        """Verify concrete capability checks take precedence over 'cap'."""
        gates = [
    ...
    def test_ungated_verdict(self):
        """Verify verdict for an unprivileged, ungated path."""
    ...
    def test_root_gated_verdict(self):
        """Verify verdict for a CAP_SYS_ADMIN root-gated path."""
        target_info, verdict, primary, _ = (
            check_privilege.analyze_target_privilege(
    ...
    def test_userns_gated_verdict(self):
        """Verify verdict for a user-namespace capability gated path."""
        target_info, verdict, primary, _ = (
            check_privilege.analyze_target_privilege(
        ...
        self.assertEqual(len(primary["gates"]), 1)
        self.assertEqual(
            primary["gates"][0]["cap_str"], "ns_capable(CAP_NET_ADMIN)"
        )

    def test_unreachable_verdict(self):
        """Verify verdict when target function has no syscall reachability."""
    ...
    def test_format_summary(self):
        """Verify human-readable summary report formatting."""
        ...
        self.assertIn(
            "Gating Status:   Ungated route available from userspace", summary
        )
        self.assertIn(
    ...
    def test_format_tree_and_paths(self):
        """Verify tree and chain path formatters with gate annotations."""
        target_info, _verdict, primary, all_res = (
            check_privilege.analyze_target_privilege(
                self.db_path, "kernel/admin.c", 50
            )
        )
        ...
        self.assertIn(
            "[Syscall: __do_sys_admin -> REACHABLE, BUT ONLY BEHIND"
            " CAP_SYS_ADMIN]",
            tree,
        )
        ...
        self.assertIn(
            "__do_sys_admin(kernel/admin.c:20)[capable(CAP_SYS_ADMIN)] ->"
            " admin_worker",
            paths_str,
        )

    def test_cli_execution(self):
        """Verify CLI execution with --file, --line, and --format summary."""
        with patch(
            "sys.argv",
            [
                "check_privilege.py",
    ...
    def test_cli_function_flag(self):
        """Verify CLI execution with --function and --format json."""
        with patch(
            "sys.argv",
            [
                "check_privilege.py",
    ...
    def test_cli_missing_db(self):
        """Verify CLI exits when required --db argument is omitted."""
        with patch(
            "sys.argv",
            ["check_privilege.py", "--file", "fs/unpriv.c", "--line", "50"],
        ):
            with self.assertRaises(SystemExit):
                check_privilege.main()

    def test_semi_gated_function_entry(self):
        """Verify entry-line analysis for a function with an internal gate."""
        target_info, verdict, primary, _ = (
            check_privilege.analyze_target_privilege(
        ...
        self.assertEqual(
            target_info["internal_gates"][0]["cap_str"],
            "capable(CAP_SYS_ADMIN)",
        )
        self.assertEqual(target_info["internal_gates"][0]["check_line"], 30)

        summary = check_privilege.format_summary(
            target_info, verdict, primary, [primary]
        )
    ...
    def test_semi_gated_interior_line(self):
        """Verify interior line after internal gate is reported as gated."""
        target_info, verdict, primary, _ = (
            check_privilege.analyze_target_privilege(
    ...
    def test_dual_path_prefers_ungated_route(self):
        """Verify Dijkstra search prefers an ungated path over a gated one."""
        target_info, verdict, primary, _ = (
            check_privilege.analyze_target_privilege(
    ...
    def test_syscall_root_directly(self):
        """Verify analyzing a syscall entry point function directly."""
        target_info, verdict, primary, _ = (
            check_privilege.analyze_target_privilege(
    ...
    def test_all_syscalls_evaluation(self):
        """Verify --all-syscalls evaluates both ungated and gated routes."""
        _target_info, verdict, _primary, all_results = (
            check_privilege.analyze_target_privilege(
    ...
    def test_json_format_serialization(self):
        """Verify JSON serialization of analysis output."""
        target_info, verdict, primary, all_results = (
            check_privilege.analyze_target_privilege(
                self.db_path, "fs/semi.c", 10, is_function_entry=True
            )
        )
        ...
        self.assertEqual(
            parsed["overall_verdict"], "REACHABLE WITH NO PRIVILEGE (UNGATED)"
        )
