"""Data-quality checks for `function_locations` (`functions.ql` output).
...
from common import is_int, report


def test_function_locations_not_empty(function_locations, kernel):
    """A full Linux kernel build contains 40,000+ functions."""
    report(
        f"function_locations rows [{kernel}]",
        {"total": len(function_locations)},
    )
    assert len(function_locations) > 10000, (
        f"only {len(function_locations)} functions extracted -- "
        "database or query severely truncated"
    )


def test_function_locations_line_ranges_are_ordered(function_locations):
    """Verify start_line <= end_line for all extracted function locations."""
    bad = [
        r
        for r in function_locations
...
def test_function_locations_core_kernel_anchors_exist(function_locations):
    ...
    report(
        "core kernel anchors",
        {
            "distinct function names": len(names),
            "missing anchors": sorted(missing),
        },
    )
    assert not missing, (
        "core kernel functions missing from function_locations: "
        f"{sorted(missing)}"
    )
