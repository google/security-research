"""Data-quality checks for the `kmalloc_calls` table (`allocations.ql`)."""
...
def test_allocations_not_empty(allocations, kernel):
    """Reference 6.1.111 DB contains 2,051 struct allocation rows."""
    report(f"kmalloc_calls rows [{kernel}]", {"total": len(allocations)})
    assert (
        len(allocations) >= 1000
    ), f"only {len(allocations)} struct allocations extracted"


def test_allocations_is_flexible_total_and_valid(allocations):
    """Verify is_flexible is populated ('true'/'false') and accurate.

    Must be populated for 100% of rows, with >0 'true' rows, and must not
    falsely mark fixed-size array element structs (iovec, bio_vec, input_event)
    ...
    invalid = [
        ...
        if r.get("is_flexible", "").lower() not in valid_vals
    ]
    flex_rows = [
        r for r in allocations if r.get("is_flexible", "").lower() == "true"
    ]
    false_flex_element_structs = {"iovec", "bio_vec", "input_event"}
    falsely_marked_flex = [
        r
        for r in flex_rows
        if r.get("struct_type") in false_flex_element_structs
    ]
    report(
    ...
    assert not falsely_marked_flex, (
        f"{len(falsely_marked_flex)} non-flexible element structs marked "
        f"is_flexible='true': {falsely_marked_flex[:5]}"
    )
    assert (
        len(flex_rows) >= 50
    ), f"only {len(flex_rows)} flexible struct allocations detected"
...
def test_allocations_struct_fits_in_alloc_size(allocations):
    ...
    Catches pointer-array struct** unwrapping and bit-layout struct/8 false
    positives.
    """
    oversized = [
        r
    ...
    report(
        "struct_size <= alloc_size", {"oversized struct rows": len(oversized)}
    )
    assert len(oversized) == 0, (
        f"{len(oversized)} rows have struct_size > alloc_size: "
        f"{oversized[:5]}"
    )


def test_allocations_struct_size_matches_btf(allocations, btf_sizes):
    """Cross-check kmalloc_calls.struct_size against BTF struct sizes."""
    named = [
        r
        for r in allocations
        if r.get("struct_type") and is_int(r.get("struct_size", ""))
    ]
    checked, matched, _ = agreement(
        named, "struct_type", "struct_size", btf_sizes
    )
    ...
    assert (
        rate >= 95.0
    ), f"struct_size agrees with BTF for only {rate:.2f}% of rows"


def test_allocations_distribution(allocations, baseline_path):
    """Verify kmalloc_calls row count does not drop below 75% of baseline."""
    ...
    report(
        "kmalloc_calls distribution",
        {
            ...
            "ratio": f"{ratio:.2f}x",
            "top structs": top_counts(allocations, "struct_type", 5),
        },
    )
    assert ratio >= 0.75, (
        f"allocations row count dropped below 75% of baseline "
        f"({len(allocations)} vs {base['rows']})"
    )
