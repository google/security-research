...
class TestExtractBTF(unittest.TestCase):
    ...
    def test_typedef_handling(self):
        """Test TYPEDEF resolution and naming for inline anonymous structs."""
        types = {
            1: {
                "id": 1,
                "kind": "INT",
    ...
    def test_type_tag_and_modifiers(self):
        ...
        types = {
            ...
            4: {
                ...
                "size": 4,
                "members": [
                    {"name": "user_ptr", "type_id": 3, "bits_offset": 0}
                ],
            },
        }

        res = get_shallow(
            types, "tagged_struct", 4, types[4]["members"][0], "tagged_struct"
        )
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0]["kind"], "INT")
    ...
    def test_array_and_flex_array(self):
        ...
        res = get_shallow(
            types, "arr_struct", 4, types[3]["members"][0], "arr_struct"
        )
    ...
    def test_enums_32_and_64(self):
        ...
        res_e32 = get_shallow(
            types, "enum_struct", 12, types[3]["members"][0], "enum_struct"
        )
        self.assertEqual(res_e32[0]["nr_bits"], 32)

        res_e64 = get_shallow(
            types, "enum_struct", 12, types[3]["members"][1], "enum_struct"
        )
    ...
    def test_bitfields(self):
        ...
        res = get_shallow(
            types, "container", 4, types[3]["members"][0], "container"
        )
    ...
    def test_func_proto(self):
        ...
        types = {
            ...
            4: {
                ...
                "size": 8,
                "members": [
                    {"name": "callback", "type_id": 3, "bits_offset": 0}
                ],
            },
        }

        res = get_shallow(
            types, "cb_struct", 8, types[4]["members"][0], "cb_struct"
        )
        self.assertEqual(len(res), 1)
        ...
        json_data = {
            "types": [
                ...
                },
                {
                    ...
                    "size": 4,
                    "members": [
                        {"name": "field_a", "type_id": 1, "bits_offset": 0}
                    ],
                },
            ]
        }
    ...
    def test_existing_btf_db_integrity(self):
        ...
        if not target_db:
            self.skipTest(
                "No btf.db database file found on disk for integration testing."
            )
        ...
        cursor.execute("""
            ...
            WHERE struct_name IS NULL OR kind IS NULL
            OR type IS NULL OR name IS NULL;
        """)
        null_count = cursor.fetchone()[0]
        self.assertEqual(
            null_count,
            0,
            f"Database {target_db} contains NULL in required fields.",
        )

        # Verify bits offset logic (bits_end >= bits_offset)
        cursor.execute(
            "SELECT count(*) FROM types WHERE bits_end < bits_offset;"
        )
        invalid_offsets = cursor.fetchone()[0]
        self.assertEqual(
            invalid_offsets, 0, f"Database {target_db} has invalid bit ranges."
        )
    ...
    def test_dump_btf_json_direct_vs_pahole_fallback(self):
        """Test dump_btf_json uses bpftool directly or pahole fallback."""
        sample_btf_json = (
            b'{"types": [{"id": 1, "kind": "INT", "name": "int", '
            b'"nr_bits": 32, "size": 4}]}'
        )

        with patch.object(
            extract_btf, "has_btf_section", return_value=True
        ), patch.object(
            extract_btf.subprocess,
            "check_output",
            return_value=sample_btf_json,
        ) as mock_check_out, patch.object(
            extract_btf.subprocess, "run"
        ) as mock_run:
        ...
        with patch.object(
            extract_btf, "has_btf_section", return_value=False
        ), patch.object(
            extract_btf.subprocess, "run", side_effect=fake_pahole
    ...
    def test_check_tools(self):
        """Test check_tools helper function to verify system binaries."""
        try:
            extract_btf.check_tools()
        except subprocess.CalledProcessError:
            self.fail(
                "check_tools() failed unexpectedly; "
                "system tools (pahole, bpftool, readelf) should be installed."
            )
    ...
    def test_flexible_array_members(self):
        ...
        types = {
            ...
            3: {
                ...
                "size": 4,
                "members": [
                    {"name": "flex_buf", "type_id": 2, "bits_offset": 32}
                ],
            },
        }

        res = get_shallow(
            types, "flex_struct", 4, types[3]["members"][0], "flex_struct"
        )
        self.assertEqual(len(res), 1)
    ...
    def test_real_vmlinux_well_known_structs(self):
        ...
        if not target_db:
            self.skipTest(
                "No btf.db database file found on disk for struct verification."
            )
        ...
        for s in well_known_structs:
            cursor.execute(
                "SELECT count(*) FROM types WHERE struct_name = ?;", (s,)
            )
            count = cursor.fetchone()[0]
            self.assertGreater(
                count, 0, f"Expected struct '{s}' not found in {target_db}."
            )

        cursor.execute(
            "SELECT type, kind, bits_offset FROM types "
    ...
    def _extract_struct_fields(self, json_data, types, struct_name):
        ...
        stype = next(
            (
                ...
                if t["kind"] == "STRUCT" and t.get("name") == struct_name
            ),
            None,
        )
        self.assertIsNotNone(stype, f"{struct_name} struct not found.")
        fields = []
        for member in stype["members"]:
            fields += get_shallow(
                types, struct_name, stype["size"], member, struct_name
            )
        return stype, fields

    def test_elastic_kernel_objects_fixture(self):
        """Test extraction of elastic kernel objects using JSON fixture."""
        fixture_path = os.path.join(
            os.path.dirname(__file__), "fixtures", "elastic_structs.json"
        )
        ...
        # 1. Test msg_msg
        msg_type, msg_fields = self._extract_struct_fields(
            json_data, types, "msg_msg"
        )
        self.assertEqual(msg_type["size"], 48)
        for expected in ["m_list.next", "m_type", "m_ts", "next", "security"]:
            self.assertIn(expected, [f["name"] for f in msg_fields])
        ...
        _, key_fields = self._extract_struct_fields(
            json_data, types, "user_key_payload"
        )
        self.assertIn("datalen", [f["name"] for f in key_fields])
        data_field = next((f for f in key_fields if f["name"] == "data"), None)
        self.assertIsNotNone(data_field)
        self.assertTrue(data_field["is_flex"])

        # 3. Test msg_msgseg and pipe_buffer
        seg_type, _ = self._extract_struct_fields(
            json_data, types, "msg_msgseg"
        )
        self.assertEqual(seg_type["size"], 8)

        _, pipe_fields = self._extract_struct_fields(
            json_data, types, "pipe_buffer"
        )
        for expected in ["page", "offset", "len", "flags"]:
            self.assertIn(expected, [f["name"] for f in pipe_fields])
