...
def filter_tracked_locations(
    ...
    """Filters function locations to those tracked in git."""
    res = subprocess.run(
    ...
    match_percentage = (
        (matched_count / total_count * 100) if total_count > 0 else 0.0
    )

    logging.info(
        "Functions in CodeQL DB: %d, "
        "Functions matched in Git repository: %d (%.2f%%)",
        total_count,
        matched_count,
        match_percentage,
    )
    ...
    if matched_count < total_count and not force:
        ...
        logging.warning(
            "CodeQL database may have been generated against a different "
            "commit! %d functions could not be found in the repository.",
            unmatched_count,
        )
        if sys.stdin.isatty():
            ans = (
            ...
            logging.error(
                "Match percentage (%.2f%%) is below 95%% and --force was "
    ...
    with tempfile.NamedTemporaryFile() as tmp:
        with tempfile.NamedTemporaryFile(delete_on_close=False) as tmp2:
            for loc in function_locations:
                tmp2.write(f"{loc[2]},{loc[3]}:{loc[1]}\n".encode("utf-8"))
            tmp2.close()
            ...
            with open(tmp.name, "w", encoding="utf-8") as out_f:
                ...
                if res.returncode != 0:
                    logging.warning(
                        "GNU parallel completed with exit status %d "
                        "(%d sub-jobs failed).",
                        res.returncode,
                        res.returncode,
                    )
...
def parse_git_log_records(repo_folder: str, log_data: list[str]) -> list:
    ...
    if not data:
        logging.critical(
            "Looks SUS no commit information has been dumped from GIT repo!"
        )
        raise ValueError
...
def create_log_table(
    ...
    """Executes parallel git log and populates the git_log table."""
    setup_git_log_schema(con)
    repo_folder = repo.git.rev_parse("--show-toplevel")

    valid_locations = filter_tracked_locations(
        repo_folder, function_locations, force
    )
...
def create_sql_db(
    ...
    """Reads function locations from CodeQL DB and creates the git_log table."""
    function_locations = []
    ...
    with closing(sqlite3.connect(db_file)) as conn:
        with conn as con:
            res = create_log_table(
                repo, no_cpu, con, function_locations, force=force
            )
            print(f"GIT Log data saved into SQLite DB. Number of lines: {res}")
...
def setup_repository(repo_dir_val: str) -> git.repo.base.Repo:
    """Opens local Git repository and unshallows history if needed."""
    ...
    try:
        ...
        if is_shallow == "true":
            logging.info(
                "Repository is shallow (--depth 1). "
                "Fetching full history (--unshallow)..."
            )
    ...
    logging.info(
        "Using repository HEAD commit: %s",
        repo.git.rev_parse("HEAD").strip(),
    )
    return repo
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Dump Git log commit metadata and function code into DB."
    )
    parser.add_argument(
        "--repo_dir",
    ...
    parser.add_argument(
        "--db_file",
        nargs="?",
        help="Path where to store Sqlite3 DB with Git Log data.",
    ...
    parser.add_argument(
        "--force",
        "-f",
        help="Force execution even if CodeQL DB function count mismatches.",
        action="store_true",
    )
    ...
    create_sql_db(
        target_db, args.codeql_db, args.no_cpu, repo, force=args.force
    )
