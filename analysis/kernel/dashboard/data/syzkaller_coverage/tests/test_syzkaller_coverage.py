#!/usr/bin/python3
"""Unit tests for syzkaller_coverage.py coverage parsing and line remapping."""
...
class TestSyzkallerCoverage(unittest.TestCase):
    """Test suite for syzkaller coverage parser and git diff line remapping."""

    def test_get_syscalls_deduplication(self):
        """Verify get_syscalls extracts syscalls and deduplicates per prog."""
        progs = [
            (
                0,
                (
                    "r0 = openat$foo(0xffffffffffffff9c, "
    ...
    def test_create_sql_db_creates_syzk_sys_and_syscalls(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT prog_id, syscall FROM syzk_sys ORDER BY syscall;"
            )
            self.assertEqual(cursor.fetchall(), [(0, "ioctl"), (0, "openat")])

            cursor.execute(
                "SELECT prog_id, syscall FROM syscalls ORDER BY syscall;"
            )
            self.assertEqual(cursor.fetchall(), [(0, "ioctl"), (0, "openat")])
    ...
    def test_extract_cov_commit(self):
        """Test auto-detection of coverage commit hash from files and headers."""
        ...
        commit = sk.extract_cov_commit(
            ...
            {
                "coverage.html": (
                    '<html><a href="https://git.kernel.org/pub/scm/linux/'
                    'kernel/git/torvalds/linux.git/commit/?id=abcdef123456">'
                    "commit</a></html>"
                )
            },
        )
    ...
    def test_parse_git_diff_u0_and_remap_line(self):
        """Test unified diff -U0 parser and line number translation."""
        diff_text = (
    ...
    def test_remap_coverage_data_end_to_end(self):
        """End-to-end test of remap_coverage_data against a temporary Git repo."""
        with tempfile.TemporaryDirectory() as repo_dir:
            ...
            with open(
                os.path.join(repo_dir, "fs/open.c"), "w", encoding="utf-8"
            ) as f:
                f.write("line1\nline2\nline3\nline4\nline5\n")
            with open(
                os.path.join(repo_dir, "net/socket.c"), "w", encoding="utf-8"
            ) as f:
                f.write("sock1\nsock2\nsock3\n")
            with open(
                os.path.join(repo_dir, "fs/obsolete.c"), "w", encoding="utf-8"
            ) as f:
                f.write("obs1\nobs2\n")

            subprocess.run(["git", "add", "."], cwd=repo_dir, check=True)
            subprocess.run(
                ["git", "commit", "-m", "v6.18 base"],
                cwd=repo_dir,
                check=True,
                stdout=subprocess.DEVNULL,
            )
            ...
            self.assertEqual(
                set(path_by_id.values()), {"fs/open.c", "net/socket.c"}
            )

            mapped_entries = {
                (path_by_id[fid], line, pid) for fid, line, pid in new_all_cov
            }
            self.assertEqual(
                mapped_entries,
                {
