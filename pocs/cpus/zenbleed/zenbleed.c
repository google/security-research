...
static void *
thread_leak_consumer (void *param)
{
  uint64_t cache[4] = { 0 };
  uint64_t total = 0;
  uint8_t *s;
...
static void *
thread_leak_producer (void *param)
{
  uint64_t cache[4] = { 0 };
...
static pthread_t
spawn_thread_core (void *(*start_routine) (void *), void *restrict arg,
		   int cpu)
{
  pthread_t tid;
  pthread_attr_t attr;
...
print_help ()
{
  ...
  logmsg
    ("   -r RNG  Restrict threads from these core numbers, e.g. 1,4,5,16-22.");
...
main (int argc, char **argv)
{
  ...
  if (variant < 0
      || variant >= sizeof (zen_leak_variants) / sizeof (*zen_leak_variants))
    {
