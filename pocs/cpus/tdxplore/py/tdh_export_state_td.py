...
from tdxtend import INVALID_PID, MBMD_SIZE, MIN_TD_STATE_EXPORT_PAGES, TdxErrorCode, TdxStatus, Tdxtend
from gateway import FOUR_KILOBYTES, Gateway


def main():
  parser = ArgumentParser(description="tdh_export_state_td")
  ...
  for i in range(MIN_TD_STATE_EXPORT_PAGES):
  ...
  print(f"export state td")

  try:
    rc, count = tdxtend.call_tdh_export_state_td(
        td.tdr_pa,
    ...
    with open(args.td_file, "wb") as f:
